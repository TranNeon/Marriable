...
//   .handler(async ({ data: { usr_msg, prjId }, context }) => {
//     // const MODEL = process.env.LLM_MODEL ?? "no model";
//     // console.log(`USER@${prjId}: ${JSON.stringify(usr_msg)}`);

//     // const llm = getLlm();
//     // console.log(`Got llm ${JSON.stringify(llm)}`);
//     //
//     console.log(`${data}, ${prjId}`);
//     return "ok";
//   });
